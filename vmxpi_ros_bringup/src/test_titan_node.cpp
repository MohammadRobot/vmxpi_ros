...
//modified by Moahmmad Alshamsi 2024
...
public:
    ...
    ros::Subscriber motor0_dist, motor1_dist, motor2_dist, motor3_dist, angle_sub, yawAngle_sub;
    ros::Subscriber enc0_sub, enc1_sub, enc2_sub, enc3_sub;
    ...
    DynamicReconfig(ros::NodeHandle *nh) {
        ...
        cmd_vel_sub =nh->subscribe("cmd_vel",1,cmdVelCallback);
    ...
    void encoder2dist()
    {
        ...
        displacey = ((leftFrontEncoder * (sqrt(3) / 2)) + ((rightFrontEncoder * (sqrt(3) / 2)) * -1)) * -0.68;
        //displacey = (leftFrontEncoder + rightFrontEncoder) / -1.5;
        //displacey = leftFrontEncoder * (sqrt(3) / 2);
        displacey /= 1000;

        //Dispace left and right
        //displacex = rightBackEncoder;
        displacex = (rightBackEncoder + (leftFrontEncoder * -0.5) + (rightFrontEncoder * -0.5)) * 0.70;
        displacex /= 1000;
    ...
    void PubDisplacements()
    {
        ...
        msg.data = motorLeftFrontSpeed;
        lmotor_PID_pub.publish(msg);

        msg.data = motorRightFrontSpeed;
        rmotor_PID_pub.publish(msg);

        msg.data = motorRightBackSpeed;
    ...
    {
        motorRightFrontSpeed = (x / 2) + (-(y * (sqrt(3) / 2))) + z;
        motorLeftFrontSpeed = (x / 2) + (y * sqrt(3) / 2) + z;
        motorRightBackSpeed = -x + z;

        double max = abs(motorRightFrontSpeed);
        if (abs(motorLeftFrontSpeed) > max)
        {
            max = abs(motorLeftFrontSpeed);
        }
        if (abs(motorRightBackSpeed) > max)
        {
            max = abs(motorRightBackSpeed);
        }
        if (max > 1)
        {
            motorRightFrontSpeed /= max;
            motorLeftFrontSpeed /= max;
            motorRightBackSpeed /= max;
        }
    }
    void DifferentialDrive(double x, double z)
    {
        motorRightFrontSpeed = -x - z; 
        motorRightBackSpeed =  -x - z;
        motorLeftFrontSpeed = x - z; 
        motorLeftBackSpeed =  x - z; 
        ...
        if (abs(motorLeftBackSpeed) > max)
        {
            max = abs(motorLeftBackSpeed);
        }
        if (max > 1)
        {
            motorRightFrontSpeed /= max;
    ...
    void setMovements(double magnitude, double Angle)
    {
        ...
        while (ros::ok())
        {
            ...
            DifferentialDrive(0.0, 0.0); //Check step to zero the motors
            ...
            DifferentialDrive(x_drive, angle_drive);
    ...
    void setAngle(double target_angle)
    {
        ...
        while (ros::ok())
        {
            ...
            if (angle < target_angle)
            {
                ...
                DifferentialDrive(0.0, -0.3); // Counter-clockwise is positive angle
            }
            else if (angle > target_angle)
            {
                //holonomicDrive(0.0, 0.0, 0.3); // Clockwise is negative angle
                DifferentialDrive(0.0, 0.3); // Clockwise is negative angle
            }
    ...
    void publish_motors()
    {
        ...
        msg1.request.speed = motorRightFrontSpeed;
        msg1.request.motor = 0;
        set_m_speed.call(msg1);

        msg1.request.speed = motorRightBackSpeed;
        msg1.request.motor = 1;
        set_m_speed.call(msg1);

        msg1.request.speed = motorLeftFrontSpeed;
        msg1.request.motor = 2;
        set_m_speed.call(msg1);

        msg1.request.speed = motorLeftBackSpeed;
        msg1.request.motor = 3;
        set_m_speed.call(msg1);
    }
    ...
    void callback(vmxpi_ros_bringup::MotorSpeedConfig &config, uint32_t level) {
        ...
        vmxpi_ros::MotorSpeed msg1;
         ...
         config.motor0_speed = ocv_motorRightFrontSpeed;
         msg1.request.speed = config.motor0_speed;
         msg1.request.motor = 0;
         set_m_speed.call(msg1);

         config.motor1_speed = ocv_motorRightFrontSpeed;
         msg1.request.speed = config.motor1_speed;
         msg1.request.motor = 1;
         set_m_speed.call(msg1);

         config.motor2_speed = ocv_motorLeftFrontSpeed;
         msg1.request.speed = config.motor2_speed;
         msg1.request.motor = 2;
         set_m_speed.call(msg1);

         config.motor3_speed = ocv_motorLeftFrontSpeed;
         msg1.request.speed = config.motor3_speed;
         msg1.request.motor = 3;
         set_m_speed.call(msg1);
         */
        DifferentialDrive(linearX, angularZ);
        publish_motors();
        //encoder2dist();
        //PubDisplacements();
        ros::Duration(0.5).sleep(); // sleep for 1 second
        //stop_motors();
