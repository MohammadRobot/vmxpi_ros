...
public:
    ...
    void DifferentialDrive(double x, double y, double z)
    {
        motorRightFrontSpeed = x/2 + y/2 + z; 
        motorLeftFrontSpeed = x/2 + y/2 + z; 
        motorRightBackSpeed = x/2 + y/2 + z; 
        motorLeftBackSpeed = x/2 + y/2 + z; 
    ...
    void setMovements(double magnitude, double Angle)
    {
        ...
        while (ros::ok())
        {
            ...
            DifferentialDrive(0.0, 0.0, 0.0); //Check step to zero the motors
            ...
            DifferentialDrive(x_drive, y_drive, angle_drive);
    ...
    void setAngle(double target_angle)
    {
        ...
        while (ros::ok())
        {
            ...
            if (angle < target_angle)
            {
                ...
                DifferentialDrive(0.0, 0.0, -0.3); // Counter-clockwise is positive angle
            }
            else if (angle > target_angle)
            {
                //holonomicDrive(0.0, 0.0, 0.3); // Clockwise is negative angle
                DifferentialDrive(0.0, 0.0, 0.3); // Clockwise is negative angle
            }
    ...
    void callback(vmxpi_ros_bringup::MotorSpeedConfig &config, uint32_t level) {
        ...
        /* vmxpi_ros::MotorSpeed msg1;
         ...
         config.motor1_speed = ocv_motorRightBackSpeed;
